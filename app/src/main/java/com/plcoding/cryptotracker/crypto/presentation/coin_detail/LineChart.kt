...
fun LineChartPreview() {
    com.plcoding.cryptotracker.ui.theme.CryptoTrackerTheme {
        val coinHistoryRandomized = androidx.compose.runtime.remember {
            (1..20).map {
                com.plcoding.cryptotracker.crypto.domain.CoinPrice(
                    priceUsd = kotlin.random.Random.Default.nextFloat() * 1000.0,
                    dateTime = java.time.ZonedDateTime.now().plusHours(it.toLong())
                )
            }
        }
        val style = ChartStyle(
            chartLineColor = androidx.compose.ui.graphics.Color.Companion.Black,
            unselectedColor = androidx.compose.ui.graphics.Color(0xFF7C7C7C),
            selectedColor = androidx.compose.ui.graphics.Color.Companion.Red,
        ...
        val dataPoints = androidx.compose.runtime.remember {
            coinHistoryRandomized.map {
                DataPoint(
                    x = it.dateTime.hour.toFloat(),
                    y = it.priceUsd.toFloat(),
                    xLabel = java.time.format.DateTimeFormatter
                        .ofPattern("ha\nM/d")
                        .format(it.dateTime)
                )
            }
